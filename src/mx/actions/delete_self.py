""" delete_self.py  -- Delete a Lifecycle's instance and state machine in deletion state """
...
def declare_mm_rvs(owner: str) -> MMRVs:
    rvs = declare_rvs(mmdb, owner, "delete_action", )
    return MMRVs(*rvs)


class DeleteSelf(ActionExecution):

    def __init__(self, action_id: str, activity_execution: "ActivityExecution"):
        """
        Perform the Delete Action on a domain model.
